...
if ($_GET['log'] && sanitize_log() && sanitize_offset() && sanitize_length() && sanitize_position())
{
	if ($log_handle = fopen($log_sources[$_GET['log']], 'r'))
	{
		...
		foreach (array_keys($log_excerpt) as $i)
		{
			...
			$log_excerpt[$i] = "<div class='log_line'><a name='" . $line_start . "'>[<a href='?log=" . $_GET['log'] . "&offset=" . max(($line_start - 4096), 0) . "&length=8192#" . $line_start . "'>" . $line_start .  "</a>] " . $log_excerpt[$i] . "</div>";
		}
	}
	else
	{
